...
namespace AJWPFAdmin.Modules.Main.ViewModels
{
    public class CarIdentificationPanelViewModel : ViewModelBase
    {
        ...
        private async Task ConfirmPassAsync(string parameter)
        {
            ...
            // 处理通道车辆统计
            var statisticRcd = ProcessPassagewayStatistic();
            ...
            if (statisticRcd != null)
            {
                statisticRcd.SippingRecordId = CurRecord.Id;
                //  扣除仓库剩余数量,
                var warehouseId = statisticRcd.WarehouseId;
            ...
            if (statisticRcd != null)
            {
                ...
                var carNo = statisticRcd.CarNo;
                var warehouseId = statisticRcd.WatchhouseId;
            ...
            // 实时通讯通知其他主机
            await _aJMQTTService.PublishAsync(AJMQTTService.CARPASSEDTOPIC, CurRecord.CarNo);
        ...
        private WarehousePassageRecord ProcessPassagewayStatistic()
        {
            ...
            if (_passagewayStatisticConfig == null)
            {
                var cfgJson = db.SystemConfigDictionaries
                    .Where(p => p.Key == Core.Enums.SystemConfigKey.PassagewayStatisticConfig)
            ...
            if (_passagewayStatisticConfig.Type == PassagewayStatisticType.Twice
                && CurRecord.Direction == Core.Enums.PassagewayDirection.出)
            {
                // 如果不存在相同仓库的在场记录,则跳出
                if (!db.ShippingRecords.OrderBy(p => p.CreateDate).Any(p => p.ShipEndDate == null
            ...
            var rcd = new WarehousePassageRecord
            {
                Id = SnowFlake.GetInstance().GetLongId(),
                CreateDate = DateTime.Now,
                CarNo = CurRecord.CarNo,
                IDCardNo = CurRecord.IDCardNo,
                Direction = CurRecord.Direction,
                PassagewayId = CurRecord.PassagewayId,
                PassagewayName = CurRecord.PassagewayName,
                SippingRecordId = 0,
                TypeId = CurRecord.TypeId,
                TypeName = CurRecord.TypeName,
                WarehouseId = CurRecord.WarehouseId,
                WarehouseName = CurRecord.WarehouseName,
                WatchhouseId = CurRecord.WatchhouseId,
                WatchhouseName = CurRecord.WatchhouseName
            };

            db.WarehousePassageRecords.Add(rcd);
